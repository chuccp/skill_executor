...
**重要：如果你决定要做某件事（如问用户问题），必须立即调用对应的工具！**
- ❌ 错误：只在思考中说"我需要问用户..."然后不调用工具
- ✅ 正确：思考后立即调用 ask_user 工具
...
**重要：如果你决定要问用户问题，必须立即调用 ask_user 工具！**
- ❌ 错误：只在思考中说"我需要问用户..."然后停下来
- ✅ 正确：思考后立即调用 ask_user 工具实际提问
...
**注意：options 数组中每个选项必须包含 value 字段！**
...
export const toolDisplayNames: Record<string, string> = {
  ...
  'git_add': 'Git添加',
  'git_commit': 'Git提交',
  'git_worktree_list': '列出Worktree',
  'git_worktree_create': '创建Worktree',
  'git_worktree_remove': '删除Worktree',
  'project_index': '索引项目',
  'find_entry_points': '查找入口',
